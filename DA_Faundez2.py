...
Py_Processing_Dir=home+"/Faundez/"


home= str(Path.home()) # Obtener el directorio raiz en cada computador distinto
BaseDir=home+"/Faundez/SUJETOS/"


Navi_df = pd.read_csv((Py_Processing_Dir+'Otras/AB_SimianMaze_Z3_NaviDataBreve_con_calculos.csv'), index_col=0)
...
#LSL_df['OW_Trial_info'] = LSL_df['OW_Trial'].apply(lambda x: trials.iloc[x])
        codex = pd.read_excel((Py_Processing_Dir+'Otras/AB_OverWatch_Codex.xlsx'),index_col=0) # Aqui estoy cargando como DataFrame la lista de códigos que voy a usar, osea, los datos del diccionario. Es super
...
# armamos como guia para hacer el reemplazo
        ...
        codex2 = pd.read_excel((Py_Processing_Dir+'Otras/AA_CODEX.xlsx'), index_col=0)
